function [xs_est,acceptance_rate]=Metropolis_multiClass_5(x_measured, mu_SAEM, Sigma_SAEM,weights_SAEM, beta_SAEM, nb_Sample_mcmc,  n, y)
...
for i=1:n
   ...
   if any(isnan(x_measured(i,:)))
     ...
     p = prb_app_obs(x_measured, mu_SAEM, Sigma_SAEM,weights_SAEM);
     
     xs_estt(i, index_miss_i,1) = generateGaussianMixture(numSamples, means, covariances, p(i,:));
     
     g                          = @(x)  densiteMelangeGaussien(x, means, covariances, p(i,:));
       ...
       switch y(i)
        ...
        case 1
           ...
           exp(beta_SAEM(1,4) + x * beta_SAEM(index_miss_i + 1,4) + x_s_1(i,index_obs_i) * beta_SAEM(index_obs_i + 1,4)))*densiteMelangeGaussien(x, means, covariances, p(i,:))));
       ...
       case 3
        ...
        t = 1;
            for s = 1: BURN + nb_Sample_mcmc
                x_s_1(i, index_miss_i, s+1) = generateGaussianMixture(numSamples, means, covariances, p(i,:));
