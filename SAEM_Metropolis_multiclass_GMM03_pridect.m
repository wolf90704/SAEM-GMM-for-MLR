function [residual_beta_estimated_SAEM, beta_estimated_SAEM,se,w_SAEM,mu_SAEM,Sigma_SAEM,z] = SAEM_Metropolis_multiclass_GMM03_pridect(X,y,n, p, beta_true ,num_components)
dim = size(X,2);
itermax_gmm = 100;
...
for j = 1:p
 ...
 [B1_mean, ~, stats_mean] = mnrfit(X_imputed, y,'model','nominal');
    beta_estimated_meanImp1 = B1_mean(:,1);
    beta_estimated_meanImp2 = B1_mean(:,2);
    beta_estimated_meanImp  =[ beta_estimated_meanImp1,beta_estimated_meanImp2]  ; 
...
for iter=1:Iter_EM
    iter
     ...
     [x_s,acceptance_rate] = Metropolis_multiClass_GMM03(X,mu_SAEM,Sigma_SAEM,w_SAEM,beta_estimated_SAEM,X_imputed,nb_Sample_MHS,n,y,z);
...
residual_beta_estimated_SAEM=beta_estimated_SAEM-beta_true;
se = stats_saem.se;
